...
.university-comparison{{background:#f8fafc;border:1px solid #dbe2ea;border-radius:10px;padding:10px 12px;margin-bottom:10px;display:flex;flex-direction:column;align-items:stretch;gap:8px}}
.comparison-top{{display:flex;align-items:center;justify-content:space-between;gap:8px}}
...
.change-badge{{display:inline-flex;align-items:center;gap:6px;font-size:12px;font-weight:700;border-radius:999px;padding:4px 8px;width:fit-content}}
.change-up{{background:#dcfce7;color:#166534}}
.change-down{{background:#fee2e2;color:#b91c1c}}
.change-flat{{background:#e2e8f0;color:#475569}}
...
.compressed-view .comparison-top{{gap:6px}}
.compressed-view .comparison-text{{display:none}}
.compressed-view .change-badge{{display:none}}
...
.school-trend-row{{display:flex;align-items:center;justify-content:space-between;gap:12px;width:100%;margin-bottom:8px}}
.school-trend-inline{{display:flex;align-items:center;gap:8px;min-width:0}}
.school-trend-canvas{{width:112px!important;height:32px!important}}
.school-trend-empty{{font-size:11px;color:#94a3b8}}
...
.trend-wrap{{margin:8px 0 12px;padding:10px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px}}
.trend-title{{font-size:12px;color:#64748b;font-weight:600;margin-bottom:6px}}
.trend-canvas{{width:100%!important;height:168px!important}}
...
<div class="guide-line">Each KPI shows a percentage. Where shown, the bracketed count is the number of items in scope for that percentage.</div>
<div class="guide-line">Comparison text states whether the faculty is above or below university, in percentage points.</div>
<div class="guide-line">Expanded school rows now include a mini trend line and a change indicator since the previous return.</div>
...
function getChangeSummary(history){{
  const cleaned = normalizeTrendHistory(history);
  if(cleaned.length < 2) return null;
  const previous = cleaned[cleaned.length - 2].percentage;
  const current = cleaned[cleaned.length - 1].percentage;
  if(typeof previous !== 'number' || typeof current !== 'number') return null;
  const delta = current - previous;
  if(Math.abs(delta) < 0.05) {{
    return {{ label: 'No change since last return', shortLabel: 'No change', className: 'change-flat', symbol: '→', delta: 0 }};
  }}
  if(delta > 0) {{
    return {{ label: `Up +${{delta.toFixed(1)}}pp since last return`, shortLabel: `+${{delta.toFixed(1)}}pp`, className: 'change-up', symbol: '↑', delta: delta }};
  }}
  return {{ label: `Down ${{delta.toFixed(1)}}pp since last return`, shortLabel: `${{delta.toFixed(1)}}pp`, className: 'change-down', symbol: '↓', delta: delta }};
}}
function createChangeBadge(history, compact=false){{
  const summary = getChangeSummary(history);
  if(!summary) return '';
  const text = compact ? summary.shortLabel : summary.label;
  return `<span class="change-badge ${{summary.className}}" title="${{summary.label}}">${{summary.symbol}} ${{text}}</span>`;
}}
...
function createSchoolTrendMarkup(history,kpiName,schoolName){{
  const cleaned = normalizeTrendHistory(history);
  if(cleaned.length < 2) return '<div class="school-trend-empty">No prior return</div>';
  const safeId = `school-trend-${{schoolName.replace(/[^a-zA-Z0-9]/g,'-')}}-${{kpiName.replace(/[^a-zA-Z0-9]/g,'-')}}`;
  const historyData = encodeURIComponent(JSON.stringify(cleaned));
  return `<div class="school-trend-inline"><canvas id="${{safeId}}" class="school-trend-canvas" data-school-history="${{historyData}}"></canvas>${{createChangeBadge(cleaned,true)}}</div>`;
}}
function renderTrendCharts(){{
  document.querySelectorAll('.trend-canvas').forEach((canvas) => {{
    ...
    const bounds = buildTrendBounds([facSeries, uniSeries]);
...
function renderSchoolTrendCharts(){{
  document.querySelectorAll('.school-trend-canvas').forEach((canvas) => {{
    if(canvas.dataset.rendered === 'true') return;
    let history = [];
    try {{
      history = JSON.parse(decodeURIComponent(canvas.getAttribute('data-school-history') || '[]'));
    }} catch (e) {{
      history = [];
    }}
    const cleaned = normalizeTrendHistory(history);
    ...
    const series = cleaned.map((point) => point.percentage);
    const bounds = buildTrendBounds([series]);
    new Chart(canvas.getContext('2d'), {{
      type: 'line',
      data: {{
        labels: cleaned.map((point) => point.date),
        datasets: [{{
          data: series,
          borderColor: '#2563eb',
          backgroundColor: 'rgba(37,99,235,0.12)',
      ...
      options: {{
        ...
        scales: {{
          x: {{ display: false }},
          y: {{ display: false, min: bounds.min, max: bounds.max }}
        }},
        elements: {{
...
function initializeFacultyDashboard(){{const facultyKpisContainer=document.getElementById('facultyKpis');facultyKpisContainer.innerHTML='';if(facultyData.kpis){{let kpiEntries=Object.entries(facultyData.kpis);if(currentSortOrder==='highLow'){{kpiEntries.sort((a,b)=>{{const aPercentage=a[1].percentage!==null?a[1].percentage:-1;const bPercentage=b[1].percentage!==null?b[1].percentage:-1;return bPercentage-aPercentage;}});}}else{{kpiEntries.sort((a,b)=>{{const aPercentage=a[1].percentage!==null?a[1].percentage:999;const bPercentage=b[1].percentage!==null?b[1].percentage:999;return aPercentage-bPercentage;}});}}for(const[kpiName,kpiData]of kpiEntries){{const card=document.createElement('div');card.className='kpi-card';card.id=`card-${{kpiName.replace(/[^a-zA-Z0-9]/g,'-')}}`;const performanceClass=getPerformanceClass(kpiData.percentage);let schoolBreakdownHtml='';const hasSchools=facultyData.schools&&Object.keys(facultyData.schools).length>0;if(hasSchools){{let schoolEntries=Object.entries(facultyData.schools).filter(([schoolName,schoolData])=>schoolData.kpis&&schoolData.kpis[kpiName]);if(currentSortOrder==='highLow'){{schoolEntries.sort((a,b)=>{{const aPerc=a[1].kpis[kpiName].percentage!==null?a[1].kpis[kpiName].percentage:-1;const bPerc=b[1].kpis[kpiName].percentage!==null?b[1].kpis[kpiName].percentage:-1;return bPerc-aPerc;}});}}else{{schoolEntries.sort((a,b)=>{{const aPerc=a[1].kpis[kpiName].percentage!==null?a[1].kpis[kpiName].percentage:999;const bPerc=b[1].kpis[kpiName].percentage!==null?b[1].kpis[kpiName].percentage:999;return aPerc-bPerc;}});}}if(schoolEntries.length>0){{schoolBreakdownHtml=`<div class="school-breakdown-content"><div class="school-breakdown-title">School Performance (${{schoolEntries.length}} schools):</div>`;for(const[schoolName,schoolData]of schoolEntries){{
    ...
    if(percentage===null||percentage===undefined||(typeof percentage==='string'&&percentage.trim()==='/')){{
        ...
        if(!isNaN(numericPercentage)){{
            ...
            if(hasZeroCount){{
                ...
                labelText=`0 ${{schoolLabel}} in scope`;
    ...
    const schoolLabel = (kpiMeta[kpiName] && kpiMeta[kpiName].denominator_label) ? kpiMeta[kpiName].denominator_label : 'records';
    const countDisplay=countValue!==null?`<div class="school-count">In scope: ${{Math.round(countValue)}} ${{schoolLabel}}</div>`:'';
    const schoolTrendHtml=createSchoolTrendMarkup(schoolKpi.history, kpiName, schoolName);
    const fillMarkup=!hasZeroCount&&barWidth>0?`<div class="school-percentage-fill" style="width:${{barWidth}}%;background:${{barColor}};"></div>`:'';
    schoolBreakdownHtml+=`<div class="school-bar-inline"><div class="school-bar-header"><div class="school-bar-name-inline">${{schoolName}}</div>${{countDisplay}}</div><div class="school-trend-row">${{schoolTrendHtml}}</div><div class="school-bar-container"><div class="${{barClass}}">${{fillMarkup}}<div class="${{labelClass}}">${{labelText}}</div></div></div></div>`;
}}
...
initializeFacultyDashboard();setTimeout(()=>{{positionTooltips();renderTrendCharts();renderSchoolTrendCharts();}},100);window.addEventListener('resize',()=>{{setTimeout(()=>{{positionTooltips();renderTrendCharts();renderSchoolTrendCharts();}},100);}});
</script>
</body>
</html>'''
